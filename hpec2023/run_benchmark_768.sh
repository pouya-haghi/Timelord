...
for ((iter=1; iter<=$max_iter; iter=iter+1))
do
    (time LD_PRELOAD=./build/libtimelord_wrapper.so ibrun ../HPCCG/test_HPCCG 100 100 100) > $data/hpccg_tl_${SLURM_NTASKS}_"${iter}".txt  2>&1
done
#
## HPL
#
...
for ((iter=1; iter<=$max_iter; iter=iter+1))
do
    (time LD_PRELOAD=./build/libtimelord_wrapper.so ibrun ../miniAMR/ref/miniAMR.x --npx 16 --npy 8 --npz 6) > $data/miniamr_tl_${SLURM_NTASKS}_"${iter}".txt  2>&1
done
